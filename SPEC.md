...
- **Tính toàn vẹn bản phát hành OTA & Chốt Chặn Tải File Trực Tuyến (Zero HTTP 404)**:
  - Tệp `version.json`, tệp binary `apk/CVA-SmartGuardian-v1.3.1.apk` và node `/app_release.json` trên Firebase RTDB trực tuyến bắt buộc phải đồng nhất 100% về `versionCode` (31), `versionName` ("1.3.1") và `sha256`.
