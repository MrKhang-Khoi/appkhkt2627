...
- **Bất biến phần cứng (Hardware Invariant) & Cơ chế Chốt Phiên Polling Độc Lập**:
  - Khi màn hình tắt (`ACTION_SCREEN_OFF`), khóa máy Keyguard (`isKeyguardLocked`), hoặc dịch vụ bị hủy (`onDestroy`), NGAY LẬP TỨC ngắt trạng thái Online và chốt phiên đếm giờ của app hiện tại. Tuyệt đối cấm đếm giờ ảo khi máy tắt màn hình hoặc khóa máy.
    ...
    - **Server-Side CAS ETag Protection (HTTP 412 Chống Ghi Đè Lỗi Thời Phía Máy Chủ)**: Bên cạnh monotonic generation fencing ở client, hệ thống tích hợp cơ chế CAS (Compare-And-Swap) chuẩn Firebase Realtime Database: mọi request cập nhật tiền cảnh `active_app.json` đều gửi kèm header `X-Firebase-ETag: true` và `if-match: <cachedETag>`. Khi phát hiện conflict (HTTP 412 Precondition Failed), hệ thống trích xuất thế hệ máy chủ (`serverGen = serverJson.optLong("foregroundGeneration")`). Nếu `serverGen >= expectedGen`, hệ thống hủy bỏ an toàn (bảo toàn trạng thái app mới hơn đã commit trên server). Nếu `serverGen < expectedGen`, hệ thống thực hiện retry một lần với ETag mới nhất từ phản hồi 412 của server. Lưu trữ `lastKnownETags` trong `ConcurrentHashMap` an toàn đa luồng.
  ...
  - Đồng bộ nguyên tử giữa RAM và đĩa: Thao tác `recordedSessionTokens.add()` và lưu SharedPreferences diễn ra nguyên tử trong cùng một khối `synchronized(statsLock)` qua hàm `recordAppSession` (được gọi độc quyền trên background coroutine `Dispatchers.IO` để đảm bảo không block luồng giao diện). Sử dụng `commit()` để đảm bảo ghi đĩa hoàn tất trước khi nhả lock; nếu `commit()` thất bại, hệ thống tự động rollback nguyên tử 100% cả in-memory SharedPreferences cache lẫn danh sách RAM tokens qua `restoreSnapshotRaw(backupTokens)` và hủy ghi nhận thời lượng, triệt tiêu hoàn toàn rủi ro dirty cache và mất đồng bộ RAM-Đĩa. Phương thức `restoreSnapshotRaw` cưỡng chế giới hạn cứng `maxEntries` bằng cách chỉ nạp tối đa `maxEntries` phần tử mới nhất từ đuôi danh sách (`subList(snapshot.size - maxEntries, snapshot.size)`), đồng thời lọc bỏ chuỗi rỗng và chuỗi vượt quá 128 ký tự để bảo toàn bất biến giới hạn bộ nhớ.
  - Khôi phục duy nhất một lần & An toàn lỗi: Biến `AtomicBoolean(false)` đảm bảo `restorePersistedSessionTokens` chỉ đánh dấu hoàn tất khi toàn bộ quá trình đọc và nạp từ SharedPreferences diễn ra thành công, ngăn chặn xáo trộn trật tự LRU và cho phép khôi phục lại nếu gặp lỗi I/O tạm thời. Tích hợp chốt chặn kích thước thô 64 KB (nếu vượt quá, tự động gỡ bỏ chuỗi độc hại khỏi SharedPreferences và trả về an toàn) và chỉ nạp tối đa 500 token gần nhất, triệt tiêu hoàn toàn nguy cơ DoS hoặc Out-Of-Memory (OOM) từ dữ liệu lưu trữ bị giả mạo.
...
- **Phạm vi kiểm thử tự động của Repository (Automated CI/Local Verification Scope)**:
  ...
  - Bộ kiểm thử này trực tiếp thực thi mã nguồn production và kiểm chứng toán học/luồng:
    ...
    12. Kiểm chứng Server-Side CAS ETag Protection với máy chủ HTTP thực tế: đảm bảo khi gói tin bị trễ đến sau gói tin mới, máy chủ trả về HTTP 412 và từ chối ghi đè, bảo toàn 100% tính toàn vẹn trạng thái tiền cảnh trên server.
    13. Kiểm chứng 4 trạng thái UI BottomSheet mutually exclusive (`testCompanionBottomSheetUiStatesAreStrictlyMutuallyExclusive`).
...
- **Nguyên tắc Đồng nhất Trạng thái Mạng (UI Anti-Contradiction & Zero-Phantom-Online Invariant)**:
  ...
  - Khi thiết bị con mất heartbeat quá thời gian chờ chuẩn (> 90s, tương thích chu kỳ heartbeat 60s và độ trễ mạng di động) hoặc đã gửi trạng thái ngắt mạng khẩn cấp: Toàn bộ banner ứng dụng đang chạy chuyển sang chế độ snapshot tĩnh với nhãn `"LẦN CUỐI GHI NHẬN TRƯỚC KHI NGOẠI TUYẾN"`, nhãn trạng thái chuyển thành `[🔴 OFFLINE]` màu đỏ cảnh báo, tuyệt đối không giữ cờ online giả từ cache Firebase.
  - **Chốt Chặn Thoát Bẫy Stale SCREEN_OFF (Stale SCREEN_OFF Deadlock Immunity)**: Trạng thái SCREEN_OFF của `active_app` chỉ được áp dụng nếu không bị nhịp tim mới hơn vượt qua (`activeTimestamp == 0L || activeTimestamp >= lastContact - 15000L`). Khi thiết bị gửi nhịp tim mới với cờ `online: true`, hệ thống triệt để giải phóng trạng thái ngoại tuyến của phiên tắt màn hình trước đó, ngăn chặn hoàn toàn lỗi khóa chết ngoại tuyến trên cả máy phụ huynh và Web Portal.
  - **Đồng Nhất Key Trạng Thái Tiền Cảnh & Fallback UsageStatsManager (Unified Foreground State Invariant)**: Cả `GuardianAccessibilityService` và `UsageTrackerService` bắt buộc phải đồng bộ hóa thống nhất ghi/xóa trên cả 2 khóa `last_foreground_pkg` và `last_active_package` (cùng các mốc thời gian `last_foreground_start` và `last_active_timestamp`), ngăn ngừa hoàn toàn tình trạng lệch khóa (key mismatch) giữa dịch vụ quét cửa sổ và chu kỳ heartbeat. Khi tạo payload telemetry `active_app`, hàm `resolveCurrentForegroundPackage` ưu tiên lấy package tiền cảnh hợp lệ; nếu rỗng hoặc vừa thức tỉnh từ `SCREEN_OFF`, hàm tự động truy vấn sự kiện `UsageStatsManager` 30 giây gần nhất (`ACTIVITY_RESUMED` / `MOVE_TO_FOREGROUND`) để tìm đúng ứng dụng tiền cảnh trước khi fallback về "HOME", triệt tiêu hoàn toàn lỗi ghi đè "HOME" khi con đang sử dụng ứng dụng khác.
